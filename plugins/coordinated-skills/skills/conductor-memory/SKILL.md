...
description: Extract and build a complete memory pack from a Claude conversation or session, synthesizing it into vault-ready artifacts so the next session resumes with full context. Produces a conversation summary, artifact index, communication-style and project-context notes, a machine-readable seed JSON and persona YAML, verification and maintenance checklists, and a generalized integration.py that copies the pack into an Obsidian vault and updates a Claude Code resume pointer (with an optional MCP/vector-store hook). Use this whenever the user says 'build conductor memory', 'extract memory from this conversation', 'capture this session', 'snapshot our progress', 'save this to memory', or 'update conductor memory' - and also when wrapping up a substantial working session whose context should persist as durable artifacts across Claude Code, Obsidian, or MCP, even if they don't say the word 'memory'. What triggers this is the ask for something written down and stored, rather than a spoken wrap-up: a plain 'let's wrap up', 'save where we are', or 'summarize the decisions we made' wants the recap ceremony and belongs to session-bookend, which reaches for this skill when the handoff needs to outlive the conversation. Built for the Conductor Method continuity system; works for any project needing durable cross-session memory.
phase: bookend
